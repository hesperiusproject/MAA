...
public class RequestHandler {
	...
	public static void start() {
		...
		BukkitScheduler scheduler = MAA.getPlugin().getServer().getScheduler();
		scheduler.scheduleAsyncRepeatingTask(MAA.getPlugin(), new Runnable() {
			public void run() {
				if (ChallengeCompletionEvent.getHandlerList().getRegisteredListeners().length == 0
					...
					try {
						...
						Bukkit.getServer().getScheduler().runTask(MAA.getPlugin(), new Runnable() {

							public void run() {
								JsonObject obj = (JsonObject) JsonParser.parseString(toGo);
	...
	public static void deleteLink(final long id) {
		Bukkit.getScheduler().runTaskAsynchronously(MAA.getPlugin(), new Runnable() {
			public void run() {
